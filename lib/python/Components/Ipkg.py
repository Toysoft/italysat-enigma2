	...
	def parseLine(self, data):
		if self.currentCommand in (self.CMD_LIST, self.CMD_UPGRADE_LIST):
			...
			if item[0].find('-settings-') > -1 and not config.plugins.softwaremanager.overwriteSettingsFiles.value:
				...
				self.excludeList.append(item)
				return
			elif item[0].find('italysat-spinner') > -1 and not config.plugins.softwaremanager.overwriteSpinnerFiles.value:
				self.excludeList.append(item)
				return
			elif item[0].find('-clone-') > -1 and not config.plugins.softwaremanager.overwriteCloneFiles.value:
                                self.excludeList.append(item)
                                return
			else:
