	...
	def quickmenuStart(self):
		try:
			if not self.session.pipshown:
				from ItalySat.ItalysatBlue import ItalysatBlue
                                self.session.open(ItalysatBlue)
			else:
				self.showExtensionSelection()
		except:
			print "[INFOBARGENERICS] QuickMenu: error pipshow, starting ItalySat Blue Panel"
			from ItalySat.ItalysatBlue import ItalysatBlue
                        self.session.open(ItalysatBlue)
		...
		if not config.plisettings.Subservice.value:
			...
			if not subservices or subservices.getNumberOfSubservices() == 0:
				if fileExists("/usr/lib/enigma2/python/Plugins/Extensions/CustomSubservices/plugin.pyo"):
					...
					from ItalySat.ItalysatGreen import ItalyGreen
                                        self.session.open(ItalyGreen)
			else:
				self.subserviceSelection()
